"""Stage 0 headroom scout for the `stage1` campaign (kernel-only weight decay).

Nothing here defines a new model. The scout reuses
``structural_reparam.experiments.reparam_sweeps100.lab.LayerwiseRepVGGCifar``
unchanged and only adds the campaign's decay-group verification gate around the
shared kernel-only-weight-decay optimizer builder.
