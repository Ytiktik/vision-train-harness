"""RepVGG block: per-branch bias-only BN (γ=1 fixed, β learnable) + single affine BN after sum."""
...
class FixedFilterBranchBNBiasOnly(nn.Module):
    """Frozen depthwise 3x3 + learned 1x1 + bias-only BN (γ=1, β learnable)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.bn(self.pointwise(y)) + self.bias[:, None, None]
    ...
    def equivalent_kernel_bias(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return (
            kernel * scale[:, None, None, None],
            -self.bn.running_mean * scale + self.bias,
        )


class RepVGGBlockBNBiasOnly(RepVGGBlockBase):
    """Per-branch bias-only BN (γ=1 fixed, β learnable); one full affine BN follows the sum.

    Isolates whether the scale term (γ) in per-branch BN matters, vs just the shift (β).
    """

    _fuse_delete_attrs = ("conv3_branches", "branch_bns", "branch_biases", "conv1", "conv1_bn", "conv1_bias", "identity_bn", "identity_bias", "fixed_branches", "bn")
    _fuse_final_bn = True
    def __init__(
        self,
        in_channels: int,
        ...
        self.branch_biases = nn.ParameterList(
            [nn.Parameter(torch.zeros(out_channels)) for _ in range(num_3x3)]
        )
        ...
        if self.has_identity:
            ...
            self.identity_bias = nn.Parameter(torch.zeros(out_channels))
        else:
            self.identity_bn = None
            self.identity_bias = None

        fixed_filters = list(fixed_filters or [])
        self.fixed_filter_names = fixed_filters
        self.fixed_branches = nn.ModuleDict(
            {
                name: FixedFilterBranchBNBiasOnly(name, in_channels, out_channels, stride)
    ...
    def _add_bias(self, bias: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self._add_bias(self.branch_biases[0], self.branch_bns[0](self.conv3_branches[0](x)))
        first_branch_out = out if self.track_distance else None
        second_branch_out: torch.Tensor | None = None
        for idx, (branch, bn, bias) in enumerate(
            zip(self.conv3_branches[1:], self.branch_bns[1:], self.branch_biases[1:]), start=1
        ):
            extra = self._add_bias(bias, bn(branch(x)))
            if idx == 1 and self.track_distance:
                second_branch_out = extra
            out = out + extra
        if self.conv1 is not None:
            out = out + self._add_bias(self.conv1_bias, self.conv1_bn(self.conv1(x)))
        if self.has_identity:
            out = out + self._add_bias(self.identity_bias, self.identity_bn(x))
    ...
    def equivalent_kernel_bias(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Sum of all branch kernels/biases after folding bias-only BN."""
        if self.deployed:
            return self.fused_conv.weight, self.fused_conv.bias

        kernel, bias = self._fold_bn_bias_only(self.conv3_branches[0], self.branch_bns[0], self.branch_biases[0])
        for branch, bn, b in zip(self.conv3_branches[1:], self.branch_bns[1:], self.branch_biases[1:]):
            k, bv = self._fold_bn_bias_only(branch, bn, b)
            kernel = kernel + k
            bias = bias + bv
        if self.conv1 is not None:
            k1, b1 = self._fold_bn_bias_only(self.conv1, self.conv1_bn, self.conv1_bias, pad=True)
            kernel = kernel + k1
            bias = bias + b1
        if self.has_identity:
            ki, bi = self._fold_identity_bn_bias_only()
            kernel = kernel + ki
            bias = bias + bi
        for branch in self.fixed_branches.values():
    ...
    def _fold_bn_bias_only(
        ...
        return kernel * scale[:, None, None, None], -bn.running_mean * scale + bias_param

    def _fold_identity_bn_bias_only(self) -> tuple[torch.Tensor, torch.Tensor]:
        bn = self.identity_bn
        scale = 1.0 / torch.sqrt(bn.running_var + bn.eps)
        ...
        return kernel * scale[:, None, None, None], -bn.running_mean * scale + self.identity_bias
