...
#
# Vendored verbatim from Apple's ml-mobileone repository
# (https://github.com/apple/ml-mobileone, mobileone.py) so the structural
# reparameterization trainer can build it via a config `target`. The only
# additions are the module docstring and this note — the model code is
# unchanged so it stays a faithful reference implementation of
# `An Improved One millisecond Mobile Backbone` (https://arxiv.org/pdf/2206.04040).
#
# Conv-structure knobs exposed to configs (see configs/mobileone_s0_imagenet.yaml):
#   num_blocks_per_stage, width_multipliers, num_conv_branches, use_se, num_classes.
#
# Training-recipe coverage vs. the paper (Sec. 4): the generic trainer applies
# cosine LR annealing, cosine weight-decay annealing (1e-4 -> 1e-5) and label
# smoothing. EMA (exponential moving average of weights, decay 0.9995) is NOT
# implemented yet — tracked in the Obsidian todo.md. Expect final accuracy a
# touch below the paper's number until EMA lands.
"""MobileOne: a multi-branch train-time / plain-CNN inference-time backbone."""
