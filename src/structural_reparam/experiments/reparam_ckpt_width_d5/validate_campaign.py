"""Campaign-level close-out validation for reparam_ckpt_width_d5 (exit criteria).
...
3. Gain coherence (INFORMATIONAL, non-gating — these are NEW cells, no
   ...
   gain @ep100 (3 seeds) ± sd, printed alongside the w1.0 anchor = the d5 cell
   of the ckpt100_*_depth_wd5e4 groups (which IS this sweep's width-1.0 point,
...
EXPECTED = {"ckpt100_c100_width_d5_wd5e4": 18, "ckpt100_c10_width_d5_wd5e4": 18}
ANCHOR = {  # w1.0 @d5 lives in the depth groups (dedup convention)
    "ckpt100_c100_width_d5_wd5e4": "ckpt100_c100_depth_wd5e4",
    "ckpt100_c10_width_d5_wd5e4": "ckpt100_c10_depth_wd5e4",
}
...
def cell_of(variant: str) -> tuple[str, str] | None:
    if variant.startswith("single_base_"):
        ...
        return None
    return arm, variant.split("_")[-1]  # e.g. ('indep_2', 'w025')
...
def main() -> None:
    ...
    print("== 3) gain table @d5 (indep_2 − single_base, pp; w1 anchor from depth group) ==")
    nan_cells = 0
    for g, runs in all_runs.items():
        g_new = gains(runs)
        anchor_runs = list(api.runs(PROJECT, filters={"group": ANCHOR[g]}))
        g_anchor = gains(anchor_runs, cells={"d5"})
        ...
        if g_anchor.get("d5"):
            mu, sd = mean_sd(list(g_anchor["d5"].values()))
            print(f"  {g} w1 (≡ {ANCHOR[g]} d5): gain {mu*100:+.2f} ± {sd*100:.2f} pp")
