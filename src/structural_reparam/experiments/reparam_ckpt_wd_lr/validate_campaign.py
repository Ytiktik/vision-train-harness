"""Campaign-level close-out validation for reparam_ckpt_wd_lr (exit criteria).

1. ALL 96 runs: finished, checkpoint/validated==1, artifact committed with
...
3. Gain parity: per cell, indep_2 − single_base mean gain @ep100 (3 seeds)
   vs the same cell in the original group (`reparam_wd_d5_*` /
   `sweeps100_*_lr_wd5e4_d5`); report Δgain vs the paired-seed SE of the
   original. (Original LR groups also carry lr=0.4 cells — absent here by
   design; the loop iterates the new groups' cells only.)
"""
...
from structural_reparam.experiments.reparam_ckpt_wd_lr.validate_first_runs import GROUP_MAP, gate_a

PROJECT = "yoovi-t-tel-aviv-university/claude-autonomous-reparam"
EXPECTED = {"ckpt100_c100_wd_d5": 24, "ckpt100_c10_wd_d5": 24,
            "ckpt100_c100_lr_wd5e4_d5": 24, "ckpt100_c10_lr_wd5e4_d5": 24}
...
def main() -> None:
    api = wandb.Api()
    print("== 1) full 96-run artifact audit ==")
    bad = 0
